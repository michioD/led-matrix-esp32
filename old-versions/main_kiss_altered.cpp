...
enum AnimationState { ANIM_OFF, ANIM_PACMAN, ANIM_KISS, ANIM_HEART, ANIM_CHIIKAWA, ANIM_BDAY, ANIM_HACHIWARE };
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  } else if (incoming == "hachiware") {
    currentState = ANIM_HACHIWARE; 
  } 
...
void reconnect() {
  // Only attempt a connection once every 5 seconds
...
void setup_wifi() {
  delay(10);
  Serial.println("\nInitializing Wi-Fi (Asynchronous)...");
  WiFi.mode(WIFI_STA);
  WiFi.disconnect(true, true);
  delay(100);
...
// void loop() {
//   // Process OTA commands only if connected
//   if (wifiMulti.run() == WL_CONNECTED) {
//     ArduinoOTA.handle();
//   }

//   // Non-blocking MQTT handler
//   if (!client.connected()) {
//     reconnect();
//   } else {
//     client.loop();
...
//   // Animation logic runs continuously
...
//       renderBday();
//     }else if (currentState == ANIM_HACHIWARE) {
//       renderHachiware();
...
void loop() {
  // 1. Process Wi-Fi and OTA
  if (wifiMulti.run() == WL_CONNECTED) {
    ArduinoOTA.handle();
    
    // 2. Process MQTT only if Wi-Fi is available
    if (!client.connected()) {
      reconnect(); // Now non-blocking
    } else {
      client.loop();
    }
  }

  // 3. Animation logic (Executes continuously regardless of network state)
  unsigned long currentMillis = millis();
